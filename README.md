...
Fetches the current US Treasury zero-yield curve from FRED
Bootstraps a bond spread by solving for the rate that prices the bond correctly against its market value
Derives a hazard rate from the spread, assuming spread reflects default risk only
Prices a CDS contract spanning from today to the bond's maturity, accounting for:
